...
/** Retail barcode we are willing to reuse or send to Shopify. Never IMEI, serial, or device code/SKU. */
...
export function collectTakenBarcodes(input: {
  inventory: Array<{ id: string; barcode?: string | null; serialImei?: string | null; specifications?: { upcSku?: string } | null }>;
  listings: Array<{ id?: string; inventoryItemId?: string; barcode?: string | null }>;
  exceptInventoryId?: string;
}): Set<string> {
  ...
  for (const item of input.inventory) {
    if (item.id === input.exceptInventoryId) continue;
    remember(item.barcode);
    remember(item.specifications?.upcSku);
  }
  for (const listing of input.listings) {
    if (listing.inventoryItemId && listing.inventoryItemId === input.exceptInventoryId) continue;
    remember(listing.barcode);
  }
  return taken;
}
...
export function resolveExistingBarcode(input: {
  ...
  const candidates = [input.inventoryBarcode, input.listingBarcode, input.upcSku];
...
export function canPrintShopifyListingLabel(listing: {
  status?: string;
  barcode?: string | null;
  ...
  return listing.status === 'active'
    ...
    && isReusableRetailBarcode(listing.barcode, { sku: listing.sku, deviceCode: listing.sku });
}
