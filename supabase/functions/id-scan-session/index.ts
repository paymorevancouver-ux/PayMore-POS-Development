...
async function handleCreate(): Promise<Response> {
  ...
  if (error) {
    ...
    return new Response(
      JSON.stringify({ error: `Failed to create session: ${error.message}` }),
...
async function handleUpload(token: string, frontImage: string, backImage?: string): Promise<Response> {
  ...
  console.log('Uploading photos for session:', token);
  ...
  try {
    ...
    if (frontUploadErr) {
      console.error('Front upload error:', frontUploadErr);
    }
    ...
    if (backImage) {
      ...
      if (backUploadErr) {
        console.error('Back upload error:', backUploadErr);
      }
    }
  } catch (uploadErr) {
    console.error('Storage upload error:', uploadErr);
    // Continue with OCR even if storage fails — we still have the base64 images
  }
  ...
  console.log('Processing ID with AI for session:', token);

  // Call OnSpace AI for OCR extraction
  let retries = 0;
  const maxRetries = 2;
  let extracted = null;
...
- Return ONLY the JSON object, no markdown, no explanation.`,
      ...
      if (!aiResponse.ok) {
        ...
        console.error(`AI attempt ${retries + 1} failed:`, lastError);
        retries++;
      ...
      if (parsed.data) {
        ...
        // Fallback: the AI returned flat structure
        extracted = parsed;
      ...
      lastError = `Parse/network error: ${(err as Error).message}`;
      console.error(`AI attempt ${retries + 1} error:`, lastError);
      retries++;
      if (retries <= maxRetries) {
        await new Promise((r) => setTimeout(r, 1000 * retries));
      }
  ...
  if (!extracted) {
    await supabase
      ...
      .update({
        status: 'error',
        error_message: `OCR extraction failed after ${maxRetries + 1} attempts. ${lastError}`,
      })
      .eq('id', session.id);
    ...
    return new Response(
      JSON.stringify({
        success: false,
        error: `Failed to extract data from ID. ${lastError}`,
      }),
  ...
  console.log('Session completed:', token);
